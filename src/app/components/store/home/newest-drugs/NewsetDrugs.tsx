...
import NewestDrugsSlider from "./Swipper";

export async function getData() {
  const token = getAuthToken();
  const res = await axios(`${BASE_url}/api/records/newestProducts`, {
    headers: {
...
export default async function NewestDrugs() {
  ...
  return (
    <div className="grid grid-cols-5 md:grid-cols-4 lg:grid-cols-5 gap-4 py-8 px-4 md:px-20 items-stretch ">
      ...
      <div className="col-span-3 md:col-span-3 lg:col-span-4">
        <div className="w-full h-full min-h-full">
          <NewestDrugsSlider products={products} />
        </div>
      </div>
    </div>
  );
